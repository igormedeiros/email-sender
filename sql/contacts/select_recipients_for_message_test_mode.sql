-- Select recipients for a message in test mode
-- Params: is_test_mode (boolean), message_id (integer)
-- Protections: Excludes unsubscribed, bounce, buyers and already sent (but allows re-sending in test mode)
...
WHERE
    -- ✅ Filter: Test mode (only contacts with 'Test' tag)
    tc.id IN (SELECT contact_id FROM test_contacts)
    ...
    -- In test mode, allow re-sending even if already sent
    AND (
        tc.id NOT IN (
        ...
        OR $1 = TRUE -- This condition is always true in this query, effectively bypassing the check
    )
